...
var session = require('../session-9698d190.cjs');
...
class PreparedQuery extends session.PreparedQuery {
    ...
    async all(placeholderValues) {
        ...
        if (fields) {
            return clientResult.then((values) => values.rows.map((row) => relations.mapResultRow(fields, row, joinsNotNullableMap)));
        }
...
function drizzle(callback, config = {}) {
    ...
    const session$1 = new SQLiteRemoteSession(callback, dialect, schema, { logger: logger$1 });
